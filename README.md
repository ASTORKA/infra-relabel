...
Рекомендуемый порядок: сначала `apply`, проверить `docker ps`, затем `images`.
...
- **Метки и проект compose** (`com.docker.compose.project/service`, имена
  сетей и томов вроде `remnanode_default`) — видны при `docker inspect`.
  Маскируются переименованием проекта/каталога (шаг B/C).
