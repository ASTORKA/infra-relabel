...
- **fleet auto-sync** *(opt-in)* — ноды флота сами держат IP друг друга в whitelist (с панели).
...
sudo SSH_PORT=22 TCP_PORTS=443,2087 UDP_PORTS=443 NODE_PORT=2222 \
     WHITELIST="1.2.3.4,2001:db8::1" NONINTERACTIVE=1 \
...
# прод-режим: пиньте тег через SG_REF — компрометация ветки main тогда не утечёт
# сразу на весь флот (скрипты тянутся из того же тега):
export SG_REF=v2.1
curl -fsSL "https://raw.githubusercontent.com/ASTORKA/infra-relabel/$SG_REF/accelerator/install.sh" | sudo -E bash -s all
```
...
| `PANEL_URL` / `PANEL_TOKEN` | _пусто_ | панель для fleet auto-sync (токен → `peers.env` 0600) |
...
```bash
sys-fw-status                 # баны, suspect, blocklist, fleet, ctguard, synproxy, CrowdSec
sys-fw-top-talkers            # топ источников по сервисным портам
nft list table inet sysguard
...
journalctl -t sys-peers-sync -t sys-blocklist -t sys-ctguard   # логи модулей
```
...
```bash
sudo PANEL_URL="https://panel.example.com" PANEL_TOKEN="ey..." \
     NONINTERACTIVE=1 bash scripts/protect.sh
# токен из панели: panel → Settings → API Tokens. Хранится в /etc/sysguard/peers.env (0600).
```
...
sudo ENABLE_CTGUARD=1 NONINTERACTIVE=1 bash scripts/protect.sh   # observe (SG_CTG_ENFORCE=0)
journalctl -t sys-ctguard         # кандидаты = только атакеры (live≤2)? тогда:
sudo ENABLE_CTGUARD=1 SG_CTG_ENFORCE=1 NONINTERACTIVE=1 bash scripts/protect.sh
```
...
> резать. Посмотри кандидатов через `sys-fw-top-talkers` и занеси upstream-диапазоны в
...
sudo bash install.sh rollback protect    # снять firewall (CrowdSec остаётся; SG_PURGE_CROWDSEC=1 чтобы удалить)
sudo bash install.sh rollback optimize   # снять тюнинг (XanMod остаётся; SG_REMOVE_XANMOD=1 + загрузка со стока чтобы удалить)
```

Бэкапы оригиналов — в `/var/backups/sysguard/<timestamp>/`.
...
- **Не `flush ruleset`.** Управляем только `inet sysguard` — CrowdSec-bouncer (`ip crowdsec`, priority −10)
