...
#   • nftables (своя таблица inet sysguard, БЕЗ flush ruleset — сосуществует с
...
#   SAFETY_DELAY=300  DRY_RUN=0  NONINTERACTIVE=1
...
# fleet auto-sync: ноды флота сами держат IP друг друга в whitelist.
# 'auto' = вкл при заданных PANEL_URL+TOKEN; 1=форс; 0=выкл.
PANEL_URL="${PANEL_URL:-}"
...
SG_CTG_ENFORCE="${SG_CTG_ENFORCE:-0}"
SG_CTG_PHANTOM_MIN="${SG_CTG_PHANTOM_MIN:-4000}"  # conntrack-порог «холдера» (выше CGNAT-churn)
SG_CTG_LIVE_FLOOR="${SG_CTG_LIVE_FLOOR:-2}"       # ≤ столько живых сокетов = фантом
SG_CTG_COARSE_MULT="${SG_CTG_COARSE_MULT:-3}"     # дамп conntrack только если ct ≥ ss×N
SG_CTG_BANTIME="${SG_CTG_BANTIME:-15m}"
SG_CTG_INTERVAL="${SG_CTG_INTERVAL:-20s}"

if [[ -t 0 && -z "${NONINTERACTIVE:-}" && "$DRY_RUN" != "1" ]]; then
...
for _k in SYN_RATE SYN_BURST UDP_RATE UDP_BURST CONN_LIMIT ICMP_RATE ICMP_BURST \
          ...
          SG_CTG_PHANTOM_MIN SG_CTG_LIVE_FLOOR SG_CTG_COARSE_MULT; do
    _is_uint "${!_k}" || { err "$_k='${!_k}' — ожидается целое число"; exit 1; }
done
for _k in SSH_BAN_TIME PORTSCAN_BAN_TIME SUSPECT_TIME SG_CTG_BANTIME; do
    _is_duration "${!_k}" || { err "$_k='${!_k}' — ожидается число с опц. суффиксом s|m|h|d"; exit 1; }
done
for _k in BLOCKLIST_REFRESH FLEET_SYNC_INTERVAL SG_CTG_INTERVAL; do
...
for _k in ENABLE_PORTSCAN_BAN ENABLE_CROWDSEC ENABLE_SYNPROXY ENABLE_BANONCE \
          ENABLE_BLOCKLISTS BLOCK_TOR ENABLE_CTGUARD SG_CTG_ENFORCE; do
...
if [[ -n "$PANEL_URL" && ! "$PANEL_URL" =~ ^https?://[A-Za-z0-9._~:/?#=%@-]+$ ]]; then
    err "PANEL_URL='$PANEL_URL' — ожидается http(s)://… без спецсимволов"; exit 1
fi
unset _k
...
arm_safety() {
    ...
    warn "Если SSH отвалится — таблица sysguard удалится через ${SAFETY_DELAY}s (доступ вернётся)."
    if command -v systemd-run >/dev/null 2>&1; then
        systemctl stop sys-fw-safety.timer 2>/dev/null || true
        systemd-run --quiet --unit=sys-fw-safety --on-active="${SAFETY_DELAY}s" \
            /usr/sbin/nft delete table inet sysguard >/dev/null 2>&1 \
            && { ok "safety: systemd-таймер sys-fw-safety на ${SAFETY_DELAY}s"; return 0; }
    fi
    ...
    mkdir -p "$STATE_DIR"
    local pidf="$STATE_DIR/sys-fw-safety.pid" logf="$STATE_DIR/sys-fw-safety.log"
    [[ -f "$pidf" && ! -L "$pidf" ]] && { kill "$(cat "$pidf")" 2>/dev/null || true; }
    nohup sh -c 'sleep "$1"; /usr/sbin/nft delete table inet sysguard 2>/dev/null; rm -f "$2"' \
...
disarm_safety() {
    systemctl stop sys-fw-safety.timer 2>/dev/null || true
    local pidf="$STATE_DIR/sys-fw-safety.pid"
    [[ -f "$pidf" && ! -L "$pidf" ]] && { kill "$(cat "$pidf")" 2>/dev/null || true; rm -f "$pidf"; }
    rm -f /tmp/sys-fw-safety.pid /tmp/sys-fw-safety.log 2>/dev/null || true   # legacy-стейт старых версий
}
...
if [[ "$NODE_PORT_WHITELIST_ONLY" == "1" ]]; then
    ...
    NODE_RULES="        # node-agent: whitelist (выше) + мягкий per-IP лимит для неизвестных
        tcp dport ${NODE_PORT} ct state new meter ctl4 { ip saddr limit rate 30/second burst 60 packets } accept
        tcp dport ${NODE_PORT} ct state new meter ctl6 { ip6 saddr limit rate 30/second burst 60 packets } accept
        tcp dport ${NODE_PORT} ct state new drop"
fi
...
# blocklist-сеты (наполняет sys-blocklist-update таймером) + drop-правило.
...
if [[ "$ENABLE_BLOCKLISTS" == "1" ]]; then
    ...
    BLOCKLIST_DROP="        # статич-блоклисты (Spamhaus DROP / FireHOL L1 [/ Tor]) — обновляет sys-blocklist-update
        ip  saddr @blocklist_v4 drop
        ip6 saddr @blocklist_v6 drop"
fi

# fleet-сеты (наполняет sys-peers-sync с панели panel) + accept сразу после whitelist.
FLEET_ON=0
case "$FLEET_SYNC" in
    1) FLEET_ON=1;;
    auto) [[ -n "$PANEL_URL" && -n "$PANEL_TOKEN" ]] && FLEET_ON=1 \
          || { [[ -f "$CONF_DIR/peers.env" ]] && FLEET_ON=1; };;
esac
FLEET_SETS=""; FLEET_ACCEPT=""
if [[ "$FLEET_ON" == "1" ]]; then
    FLEET_SETS="    set sys_peers_v4 { type ipv4_addr; flags interval; auto-merge; }
    set sys_peers_v6 { type ipv6_addr; flags interval; auto-merge; }"
    FLEET_ACCEPT="        # ноды флота (авто-синк с панели) — свои серверы, обходят все лимиты
        ip  saddr @sys_peers_v4 accept
        ip6 saddr @sys_peers_v6 accept"
fi
...
# ─── Генерация nft-файла ─────────────────────────────────────────────────────
NFT_FILE="$CONF_DIR/sysguard.nft"
[[ "$DRY_RUN" == "1" ]] && NFT_FILE="$(mktemp /tmp/sysguard.XXXXXX.nft)"
mkdir -p "$CONF_DIR"
title "Генерация nftables → $NFT_FILE"

cat > "$NFT_FILE" <<NFT
#!/usr/sbin/nft -f
# sysguard / protect.sh @ $(date -Is)
# Управляем ТОЛЬКО своей таблицей — НЕ flush ruleset (живём рядом с CrowdSec/Docker).

table inet sysguard {}
delete table inet sysguard
...
arm_safety
nft -f "$NFT_FILE"
ok "nftables sysguard применён"

# boot-persist через свой сервис (не трогаем /etc/nftables.conf и чужие таблицы)
cat > /etc/systemd/system/sys-firewall.service <<EOF
[Unit]
Description=system firewall (nftables)
...
if [[ "$SYNPROXY_OK" == "1" ]]; then
    echo "nf_synproxy" > /etc/modules-load.d/sys-synproxy.conf
else
    rm -f /etc/modules-load.d/sys-synproxy.conf 2>/dev/null || true
fi
systemctl daemon-reload
systemctl enable sys-firewall.service >/dev/null 2>&1 || true
systemctl enable nftables >/dev/null 2>&1 || true
ok "sys-firewall.service включён (правила переживут reboot)"
...
if [[ "$ENABLE_CROWDSEC" == "1" ]]; then
    ...
    if command -v cscli >/dev/null 2>&1; then
        ...
        if [[ -n "$IP_ITEMS$CIDR_ITEMS" ]]; then
            {
                echo "name: sysguard/whitelist"
                echo "description: never ban admin/panel"
                echo "whitelist:"
                echo "  reason: sysguard trusted"
                [[ -n "$IP_ITEMS"   ]] && { echo "  ip:";   printf "%s" "$IP_ITEMS"; }
                [[ -n "$CIDR_ITEMS" ]] && { echo "  cidr:"; printf "%s" "$CIDR_ITEMS"; }
            } > /etc/crowdsec/parsers/s02-enrich/sys-whitelist.yaml
        else
            rm -f /etc/crowdsec/parsers/s02-enrich/sys-whitelist.yaml
        fi

        # источник логов sshd через journald (на системах без /var/log/auth.log)
        mkdir -p /etc/crowdsec/acquis.d
        cat > /etc/crowdsec/acquis.d/sys-sshd.yaml <<'ACQ'
source: journalctl
journalctl_filter:
...
# ── Fleet auto-sync: ноды флота из панели → nft-сет sys_peers_* ──────
if [[ "$FLEET_ON" == "1" ]]; then
    title "Fleet auto-sync (ноды флота → whitelist)"
    if [[ -n "$PANEL_URL" && -n "$PANEL_TOKEN" ]]; then
        umask 077; mkdir -p "$CONF_DIR"
        printf 'PANEL_URL=%s\nPANEL_TOKEN=%s\n' "$PANEL_URL" "$PANEL_TOKEN" > "$CONF_DIR/peers.env"
        chmod 0600 "$CONF_DIR/peers.env"; chown root:root "$CONF_DIR/peers.env" 2>/dev/null || true
        ok "токен панели сохранён в $CONF_DIR/peers.env (root:root 0600, НЕ в protect.conf)"
    elif [[ -f "$CONF_DIR/peers.env" ]]; then
        info "использую сохранённый $CONF_DIR/peers.env"
    fi
    cat > /usr/local/sbin/sys-peers-sync <<'FSYNC'
#!/usr/bin/env bash
# sys-peers-sync — тянет адреса нод флота из panel (GET /api/nodes по Bearer) и
# держит их в nft-сете sys_peers_v4/v6 (accept сразу после whitelist). Токен уходит
# ТОЛЬКО на заданный оператором $PANEL_URL. Fail-safe: панель недоступна / кривой
# ответ / 0 валидных IP → текущий whitelist нод НЕ трогаем (last-known-good). Применение
# отдельной nft-транзакцией: битые данные не ломают sysguard.
set -u
TAG=sys-peers-sync
ENVF=/etc/sysguard/peers.env
[ -r "$ENVF" ] || { logger -t "$TAG" "нет $ENVF — выкл"; exit 0; }
# shellcheck disable=SC1090
. "$ENVF"
URL="${PANEL_URL:-}"; TOKEN="${PANEL_TOKEN:-}"
...
nft list set inet sysguard sys_peers_v4 >/dev/null 2>&1 || { logger -t "$TAG" "сет sys_peers нет (protect без fleet) — выкл"; exit 0; }
...
{
    echo "flush set inet sysguard sys_peers_v4"
    [ -n "$V4" ] && echo "add element inet sysguard sys_peers_v4 { $V4 }"
    echo "flush set inet sysguard sys_peers_v6"
    [ -n "$V6" ] && echo "add element inet sysguard sys_peers_v6 { $V6 }"
} > "$TMP/upd.nft"
n4=$(printf '%s' "$V4" | tr ',' '\n' | grep -c . || true)
n6=$(printf '%s' "$V6" | tr ',' '\n' | grep -c . || true)
...
FSYNC
    chmod +x /usr/local/sbin/sys-peers-sync
    cat > /etc/systemd/system/sys-peers-sync.service <<'EOF'
[Unit]
Description=peer allowlist sync
After=sys-firewall.service network-online.target
Wants=network-online.target
[Service]
Type=oneshot
ExecStart=/usr/local/sbin/sys-peers-sync
EOF
    cat > /etc/systemd/system/sys-peers-sync.timer <<EOF
[Unit]
Description=peer allowlist sync timer
[Timer]
OnBootSec=60s
...
EOF
    systemctl daemon-reload
    systemctl enable --now sys-peers-sync.timer >/dev/null 2>&1 || true
    /usr/local/sbin/sys-peers-sync >/dev/null 2>&1 || true
    ok "fleet-sync включён (интервал $FLEET_SYNC_INTERVAL). Лог: journalctl -t sys-peers-sync"
fi
...
if [[ "$ENABLE_BLOCKLISTS" == "1" ]]; then
    ...
    cat > /usr/local/sbin/sys-blocklist-update <<'BLUP'
#!/usr/bin/env bash
# sys-blocklist-update — обновляет nft-сеты blocklist_v4/v6 из публичных threat-фидов.
# Источники: Spamhaus DROP (json v4+v6), FireHOL Level 1 (v4), опц. Tor exit-list.
# Плюс /etc/sysguard/custom-blocklist.txt (локальные дополнения оператора).
# Bogon/private-фильтр, валидация, отдельная nft-транзакция (битый фид не ломает
# sysguard), last-known-good при недоступности фидов.
set -u
TAG=sys-blocklist
BLOCK_TOR_FLAG="${1:-0}"
CUSTOM=/etc/sysguard/custom-blocklist.txt
nft list set inet sysguard blocklist_v4 >/dev/null 2>&1 || { logger -t "$TAG" "сет blocklist нет — выкл"; exit 0; }
command -v curl >/dev/null 2>&1 || { logger -t "$TAG" "нет curl"; exit 1; }
TMP="$(mktemp -d)"; trap 'rm -rf "$TMP"' EXIT
...
{
    echo "flush set inet sysguard blocklist_v4"
    echo "add element inet sysguard blocklist_v4 { $(paste -sd, "$TMP/v4.clean") }"
    if [ "$N6" -gt 0 ]; then
        echo "flush set inet sysguard blocklist_v6"
        echo "add element inet sysguard blocklist_v6 { $(paste -sd, "$TMP/v6.clean") }"
    fi
} > "$TMP/bl.nft"
...
BLUP
    chmod +x /usr/local/sbin/sys-blocklist-update
    cat > /etc/systemd/system/sys-blocklist.service <<EOF
[Unit]
Description=threat blocklist update
After=sys-firewall.service network-online.target
Wants=network-online.target
[Service]
Type=oneshot
ExecStart=/usr/local/sbin/sys-blocklist-update $BLOCK_TOR
EOF
    cat > /etc/systemd/system/sys-blocklist.timer <<EOF
[Unit]
Description=blocklist refresh timer
[Timer]
OnBootSec=120s
...
EOF
    systemctl daemon-reload
    systemctl enable --now sys-blocklist.timer >/dev/null 2>&1 || true
    /usr/local/sbin/sys-blocklist-update "$BLOCK_TOR" >/dev/null 2>&1 || true
    ok "блоклисты включены (обновление $BLOCKLIST_REFRESH). Лог: journalctl -t sys-blocklist"
fi

# ── conntrack phantom-eviction (защита от distributed connect-and-hold) ───────
if [[ "$ENABLE_CTGUARD" == "1" ]]; then
    title "conntrack-guard (phantom-eviction)$([[ "$SG_CTG_ENFORCE" == "1" ]] && echo ' [ENFORCE]' || echo ' [observe]')"
    cat > "$CONF_DIR/ctguard.conf" <<EOF
# sysguard ctguard — детект distributed connect-and-hold по «живым» сокетам.
...
SG_CTG_ENFORCE=$SG_CTG_ENFORCE
SG_CTG_PHANTOM_MIN=${SG_CTG_PHANTOM_MIN:-4000}
SG_CTG_LIVE_FLOOR=${SG_CTG_LIVE_FLOOR:-2}
SG_CTG_BANTIME=${SG_CTG_BANTIME:-15m}
SG_CTG_COARSE_MULT=${SG_CTG_COARSE_MULT:-3}
EOF
    chmod 0640 "$CONF_DIR/ctguard.conf"
    cat > /usr/local/sbin/sys-ctguard <<'CTG'
#!/usr/bin/env bash
# sys-ctguard — liveness-aware защита от distributed connect-and-hold флуда. Класс атаки,
...
# малым conntrack или в whitelist. observe-режим (SG_CTG_ENFORCE=0) — только лог.
set -u
TAG=sys-ctguard
CONF=/etc/sysguard/ctguard.conf
# shellcheck disable=SC1090
[ -r "$CONF" ] && . "$CONF"
ENFORCE="${SG_CTG_ENFORCE:-0}"
PHANTOM_MIN="${SG_CTG_PHANTOM_MIN:-4000}"
LIVE_FLOOR="${SG_CTG_LIVE_FLOOR:-2}"
BANTIME="${SG_CTG_BANTIME:-15m}"
COARSE_MULT="${SG_CTG_COARSE_MULT:-3}"
command -v conntrack >/dev/null 2>&1 || { logger -t "$TAG" "нет conntrack-tools"; exit 0; }

# своя изолированная таблица (priority -5 → раньше sysguard); rollback = удалить таблицу
nft list table inet sys_ctguard >/dev/null 2>&1 || nft -f - <<'NFTG'
table inet sys_ctguard {
...
is_white() {  # в whitelist sysguard или в fleet-сете?
    local ip="$1" s4 s6
    if printf '%s' "$ip" | grep -q ':'; then s4=whitelist_v6; s6=sys_peers_v6; else s4=whitelist_v4; s6=sys_peers_v4; fi
    nft get element inet sysguard "$s4" "{ $ip }" >/dev/null 2>&1 && return 0
    nft get element inet sysguard "$s6" "{ $ip }" >/dev/null 2>&1 && return 0
    return 1
}
cand=0; eict=0
while read -r cnt ip; do
    ...
    if [ "$ENFORCE" = "1" ]; then
        ...
        nft add element inet sys_ctguard "$setn" "{ $ip timeout $BANTIME }" 2>/dev/null \
            && conntrack -D -s "$ip" >/dev/null 2>&1 && eict=$((eict+1))
        logger -t "$TAG" "evict $ip ct=$cnt live=$live (bantime $BANTIME)"
    else
        logger -t "$TAG" "[observe] phantom-кандидат $ip ct=$cnt live=$live (SG_CTG_ENFORCE=0 — без эвикта)"
    fi
done < "$TMP/ct"
[ "$cand" -gt 0 ] && logger -t "$TAG" "тик: ct_total=$CT_TOTAL ss=$SS_TOTAL кандидатов=$cand эвиктов=$eict enforce=$ENFORCE"
exit 0
CTG
    chmod +x /usr/local/sbin/sys-ctguard
    cat > /etc/systemd/system/sys-ctguard.service <<'EOF'
[Unit]
Description=conntrack eviction guard
After=sys-firewall.service
[Service]
Type=oneshot
# не отбираем CPU у xray под атакой
Nice=10
IOSchedulingClass=idle
ExecStart=/usr/local/sbin/sys-ctguard
EOF
    cat > /etc/systemd/system/sys-ctguard.timer <<EOF
[Unit]
Description=conntrack guard timer
[Timer]
OnBootSec=90s
OnUnitActiveSec=${SG_CTG_INTERVAL:-20s}
[Install]
WantedBy=timers.target
EOF
    systemctl daemon-reload
    systemctl enable --now sys-ctguard.timer >/dev/null 2>&1 || true
    if [[ "$SG_CTG_ENFORCE" == "1" ]]; then
        ok "ctguard ENFORCE: фантом-холдеры эвиктятся. Лог: journalctl -t sys-ctguard"
    else
        warn "ctguard в OBSERVE (только лог). Убедись по journalctl -t sys-ctguard, что кандидаты = только атакеры (live≤$SG_CTG_LIVE_FLOOR), затем SG_CTG_ENFORCE=1 + ре-ран protect."
    fi
fi

# ─── fw-status хелпер ────────────────────────────────────────────────────────
cat > /usr/local/sbin/sys-fw-status <<'STAT'
#!/usr/bin/env bash
echo "── nft table inet sysguard ──"
nft list table inet sysguard 2>/dev/null | grep -E 'policy|counter|elements' | head -40
echo
echo "── autoban (живые баны) ──"
echo "v4: $(nft list set inet sysguard autoban_v4 2>/dev/null | grep -oE '[0-9.]+ timeout' | wc -l)   v6: $(nft list set inet sysguard autoban_v6 2>/dev/null | grep -c timeout)"
nft list set inet sysguard autoban_v4 2>/dev/null | grep -oE '[0-9.]+ (timeout|expires)[^,]*' | head -15
if nft list set inet sysguard suspect_v4 >/dev/null 2>&1; then
    echo "suspect (наблюдение, ban-once) v4: $(nft list set inet sysguard suspect_v4 2>/dev/null | grep -c timeout)   v6: $(nft list set inet sysguard suspect_v6 2>/dev/null | grep -c timeout)"
fi
echo
if nft list set inet sysguard blocklist_v4 >/dev/null 2>&1; then
    echo "── threat-блоклисты ──"
    echo "v4: $(nft list set inet sysguard blocklist_v4 2>/dev/null | grep -coE '[0-9.]+')   v6: $(nft list set inet sysguard blocklist_v6 2>/dev/null | grep -c ':')   (обновляет sys-blocklist-update)"
    echo
fi
if nft list set inet sysguard sys_peers_v4 >/dev/null 2>&1; then
    echo "── fleet-sync (ноды флота → whitelist) ──"
    echo "v4: $(nft list set inet sysguard sys_peers_v4 2>/dev/null | grep -coE '[0-9.]+')   v6: $(nft list set inet sysguard sys_peers_v6 2>/dev/null | grep -c ':')   (последний синк: $(journalctl -t sys-peers-sync -n1 --no-pager -o cat 2>/dev/null | head -c 80))"
    echo
fi
if nft list table inet sys_ctguard >/dev/null 2>&1; then
    echo "── ctguard (phantom-eviction) ──"
    enf="$(awk -F= '/^SG_CTG_ENFORCE/{print $2}' /etc/sysguard/ctguard.conf 2>/dev/null)"
    echo "режим: $([ "${enf:-0}" = 1 ] && echo ENFORCE || echo observe)   фантомов в блоке v4: $(nft list set inet sys_ctguard phantom_v4 2>/dev/null | grep -c timeout)   v6: $(nft list set inet sys_ctguard phantom_v6 2>/dev/null | grep -c timeout)"
    journalctl -t sys-ctguard -n3 --no-pager -o cat 2>/dev/null | sed 's/^/    /'
    echo
fi
if [ -f /var/lib/sysguard/.synproxy-degraded ]; then
    echo "⚠ SYNPROXY DEGRADED: $(cat /var/lib/sysguard/.synproxy-degraded)"
    echo
fi
if command -v cscli >/dev/null 2>&1; then
    echo "── CrowdSec ──"
...
STAT
chmod +x /usr/local/sbin/sys-fw-status
...
cat > /usr/local/sbin/sys-fw-top-talkers <<'TT'
#!/usr/bin/env bash
...
#   sys-fw-top-talkers [порт[,порт...]] [N]   (по умолчанию порты из protect, N=25)
set -u
DEF=443
if [ -r /var/lib/sysguard/protect.installed ]; then
    DEF="$(awk -F= '/^tcp_ports=/{print $2}' /var/lib/sysguard/protect.installed)"
fi
PORTS="${1:-${DEF:-443}}"
N="${2:-25}"
filt=""
...
TT
chmod +x /usr/local/sbin/sys-fw-top-talkers
...
# PANEL_URL/TOKEN сюда НЕ пишем — токен живёт в peers.env (0600), fleet-режим
# восстанавливается по наличию peers.env.
save_conf "$CONF_DIR/protect.conf" \
    ...
    ENABLE_CTGUARD SG_CTG_ENFORCE SG_CTG_PHANTOM_MIN SG_CTG_LIVE_FLOOR \
    SG_CTG_COARSE_MULT SG_CTG_BANTIME SG_CTG_INTERVAL
...
if [[ -t 0 && -z "${NONINTERACTIVE:-}" ]]; then
    ...
    if [[ "$c" =~ ^[yYдД] ]]; then
        ...
        warn "Сейфти оставлен: через ${SAFETY_DELAY}s sysguard удалится сам."
        warn "Если всё ок — сними: systemctl stop sys-fw-safety.timer  (или kill из /tmp/sys-fw-safety.pid)"
    fi
else
    warn "Неинтерактивно: сейфти-таймер на ${SAFETY_DELAY}s АКТИВЕН."
    warn "Подтверди доступ и сними: systemctl stop sys-fw-safety.timer"
fi
echo
ok "Готово. Статус: sys-fw-status | топ источников (для WHITELIST за CDN/LB): sys-fw-top-talkers"
