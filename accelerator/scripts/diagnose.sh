...
emit_json() {
    ...
    nft list table inet sysguard >/dev/null 2>&1 && fw=true || fw=false
    ab4="$(nft list set inet sysguard autoban_v4 2>/dev/null | grep -c timeout)"
    ab6="$(nft list set inet sysguard autoban_v6 2>/dev/null | grep -c timeout)"
    susp="$(nft list set inet sysguard suspect_v4 2>/dev/null | grep -c timeout)"
    bl4="$(nft list set inet sysguard blocklist_v4 2>/dev/null | grep -coE '[0-9.]+')"
    bl6="$(nft list set inet sysguard blocklist_v6 2>/dev/null | grep -c ':')"
    fl4="$(nft list set inet sysguard sys_peers_v4 2>/dev/null | grep -coE '[0-9.]+')"
    fl6="$(nft list set inet sysguard sys_peers_v6 2>/dev/null | grep -c ':')"
    ...
    if nft list table inet sys_ctguard >/dev/null 2>&1; then
        local enf; enf="$(awk -F= '/^SG_CTG_ENFORCE/{print $2}' /etc/sysguard/ctguard.conf 2>/dev/null)"
        [[ "${enf:-0}" == 1 ]] && ctg=enforce || ctg=observe
    else ctg=off; fi
    [[ -f "$STATE_DIR/.synproxy-degraded" ]] && syndeg=true || syndeg=false
    { systemctl is-active --quiet sys-fw-safety.timer 2>/dev/null \
      || { [[ -f "$STATE_DIR/sys-fw-safety.pid" ]] && kill -0 "$(cat "$STATE_DIR/sys-fw-safety.pid" 2>/dev/null)" 2>/dev/null; }; } \
...
if [[ -n "$NIC" ]]; then
    ...
    systemctl is-active --quiet sys-rps.service 2>/dev/null && pass "sys-rps.service активен" || info "sys-rps.service не запущен (ставится оптимизатором)"
else
...
if nft list table inet sysguard >/dev/null 2>&1; then
    pass "nftables sysguard активна (policy drop на input)"
    AB4=$(nft list set inet sysguard autoban_v4 2>/dev/null | grep -c 'timeout\|expires')
    AB6=$(nft list set inet sysguard autoban_v6 2>/dev/null | grep -c 'timeout\|expires')
    info "autoban: v4=$AB4  v6=$AB6"
    WLN=$(nft list set inet sysguard whitelist_v4 2>/dev/null | grep -oE '[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+' | wc -l)
    ...
    CLIM=$(nft list chain inet sysguard input 2>/dev/null | grep -oE 'ct count over [0-9]+' | head -1 | grep -oE '[0-9]+')
    if [[ -n "$CLIM" ]]; then
    ...
    if nft list set inet sysguard blocklist_v4 >/dev/null 2>&1; then
        BL4=$(nft list set inet sysguard blocklist_v4 2>/dev/null | grep -coE '[0-9.]+')
        [[ "$BL4" -gt 0 ]] && pass "threat-блоклисты: v4=$BL4 записей (sys-blocklist-update)" \
            || wrn "blocklist_v4 пуст — фиды не подтянулись? journalctl -t sys-blocklist"
    fi
    if nft list set inet sysguard sys_peers_v4 >/dev/null 2>&1; then
        FL4=$(nft list set inet sysguard sys_peers_v4 2>/dev/null | grep -coE '[0-9.]+')
        [[ "$FL4" -gt 0 ]] && pass "fleet-sync: $FL4 нод флота в whitelist" \
            || wrn "sys_peers пуст — панель/токен? journalctl -t sys-peers-sync"
    fi
else
    wrn "sysguard не активна — защита не стоит (запусти 🛡 protect)"
fi
# ctguard (отдельная таблица)
if nft list table inet sys_ctguard >/dev/null 2>&1; then
    ENF=$(awk -F= '/^SG_CTG_ENFORCE/{print $2}' /etc/sysguard/ctguard.conf 2>/dev/null)
    PH4=$(nft list set inet sys_ctguard phantom_v4 2>/dev/null | grep -c timeout)
    [[ "${ENF:-0}" == 1 ]] && pass "ctguard ENFORCE активен (фантомов в блоке: $PH4)" \
        || info "ctguard в observe-режиме (только лог; SG_CTG_ENFORCE=1 для эвикта)"
fi
...
# Взведённый сейфти-таймер: protect в неинтерактиве оставляет sys-fw-safety активным.
# Если не снять — sysguard САМОУДАЛИТСЯ через SAFETY_DELAY. Ловим это громко.
if systemctl is-active --quiet sys-fw-safety.timer 2>/dev/null \
   || { [[ -f "$STATE_DIR/sys-fw-safety.pid" ]] && kill -0 "$(cat "$STATE_DIR/sys-fw-safety.pid" 2>/dev/null)" 2>/dev/null; } \
   || { [[ -f /tmp/sys-fw-safety.pid ]] && kill -0 "$(cat /tmp/sys-fw-safety.pid 2>/dev/null)" 2>/dev/null; }; then
    bad "ВЗВЕДЁН сейфти-таймер sys-fw-safety — sysguard СКОРО САМОУДАЛИТСЯ! Сними после проверки доступа: systemctl stop sys-fw-safety.timer"
fi
if command -v cscli >/dev/null 2>&1; then
    ...
    nft list table ip crowdsec >/dev/null 2>&1 && info "таблица bouncer'а ip crowdsec присутствует (priority -10, раньше sysguard)"
else
