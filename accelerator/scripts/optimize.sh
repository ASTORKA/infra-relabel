...
backup_file /etc/sysctl.d/99-sysguard.conf "$BACKUP"
cat > /etc/sysctl.d/99-sysguard.conf <<SYSCTL
# === sysguard / optimize (RAM-tier $TIER, ~${_mem_mb} MB) ===
...
# RPS: глобальная таблица flow-привязок (дополняет per-queue настройку из sys-rps)
...
# масштабируется от RAM (99-sysguard-conntrack.conf), чтобы мелкая VPS под
...
CT_BUCKETS=$(( CT_MAX / 4 ))
cat > /etc/sysctl.d/99-sysguard-conntrack.conf <<CT
# sysguard: ёмкость conntrack под RAM этой ноды (~$(( _mem_kb / 1024 )) MB)
...
echo "tcp_bbr"      > /etc/modules-load.d/sys-bbr.conf
echo "nf_conntrack" > /etc/modules-load.d/sys-conntrack.conf
...
sed -i '/# === sysguard ===/,/# === \/sysguard ===/d' /etc/security/limits.conf
...
cat > /etc/systemd/system.conf.d/sys-limits.conf <<'L'
[Manager]
DefaultLimitNOFILE=1048576
DefaultLimitNPROC=1048576
L
cp /etc/systemd/system.conf.d/sys-limits.conf /etc/systemd/user.conf.d/sys-limits.conf
...
cat > /usr/local/sbin/sys-rps-setup <<'RPS'
#!/usr/bin/env bash
...
echo "sys-rps: NIC=$NIC mask=$mask cpus=$ncpu"
RPS
chmod +x /usr/local/sbin/sys-rps-setup

cat > /etc/systemd/system/sys-rps.service <<'EOF'
[Unit]
Description=sysguard RPS/RFS/XPS tuning
...
Type=oneshot
RemainAfterExit=yes
ExecStart=/usr/local/sbin/sys-rps-setup

[Install]
WantedBy=multi-user.target
EOF
systemctl daemon-reload
systemctl enable --now sys-rps.service >/dev/null 2>&1 || true
...
EOF
    if nft -c -f "$CONF_DIR/sys_mss.nft" 2>/dev/null; then
        cat > /etc/systemd/system/sys-mss-clamp.service <<EOF
[Unit]
Description=sysguard MSS clamp to PMTU (forward)
...
Type=oneshot
RemainAfterExit=yes
ExecStart=/usr/sbin/nft -f $CONF_DIR/sys_mss.nft
ExecStop=/usr/sbin/nft delete table inet sys_mss

[Install]
WantedBy=multi-user.target
EOF
        systemctl daemon-reload
        systemctl enable --now sys-mss-clamp.service >/dev/null 2>&1 || true
...
if swapon --show 2>/dev/null | grep -q .; then
    ...
    cat > /usr/local/sbin/sys-zram-setup <<'ZR'
#!/usr/bin/env bash
...
echo "sys-zram: $DEV size=$SIZE"
ZR
    chmod +x /usr/local/sbin/sys-zram-setup
    cat > /etc/systemd/system/sys-zram.service <<'EOF'
[Unit]
Description=sysguard zram-swap
After=local-fs.target
[Service]
Type=oneshot
RemainAfterExit=yes
ExecStart=/usr/local/sbin/sys-zram-setup
ExecStop=/bin/sh -c 'for d in $(swapon --show=NAME --noheadings 2>/dev/null | grep /dev/zram); do swapoff "$d" 2>/dev/null || true; done'
[Install]
WantedBy=multi-user.target
EOF
    systemctl daemon-reload
    systemctl enable --now sys-zram.service >/dev/null 2>&1 || true
...
cat > /etc/systemd/journald.conf.d/sys-size.conf <<'J'
[Journal]
SystemMaxUse=300M
SystemKeepFree=500M
...
cat > /etc/systemd/system/sys-thp-off.service <<'EOF'
[Unit]
Description=sysguard disable THP
...
EOF
systemctl daemon-reload
systemctl enable --now sys-thp-off.service >/dev/null 2>&1 || true
ok "THP отключён"

# ─── 10. CPU governor ────────────────────────────────────────────────────────
title "CPU governor → performance"
if [[ -d /sys/devices/system/cpu/cpu0/cpufreq ]]; then
    cat > /etc/systemd/system/sys-cpu-perf.service <<'EOF'
[Unit]
Description=sysguard CPU governor performance
After=multi-user.target
...
EOF
    systemctl daemon-reload
    systemctl enable --now sys-cpu-perf.service >/dev/null 2>&1 || true
