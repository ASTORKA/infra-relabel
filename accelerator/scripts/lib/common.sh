#!/usr/bin/env bash
# Общие функции для sysguard (⚡ оптимизатор + 🛡 защита + 🩺 диагностика).
# Namespace: всё наше живёт под префиксом "sys-" / "sys_" чтобы не конфликтовать
...
STATE_DIR=/var/lib/sysguard
CONF_DIR=/etc/sysguard
...
save_conf() {
    ...
    tmp="$(mktemp "${TMPDIR:-/tmp}/sys-conf.XXXXXX")" || return 1
    {
        echo "# sysguard — сохранённый конфиг ноды @ $(date -Is)"
