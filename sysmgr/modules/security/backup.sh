...
SYSCTL_CONF_FILE="/etc/sysctl.d/99-sysmgr-hardening.conf"
...
_backup_create() {
    ...
    local backup_name="sysmgr-security-backup-$(date +%Y%m%d_%H%M%S)"
    ...
    local files_to_backup=(
        ...
        "$SYSCTL_CONF_FILE"
        "$F2B_WHITELIST_FILE"
        "/etc/sysmgr/global-whitelist.txt"
        "/etc/sysmgr/geoblock/countries.txt"
        "${SCRIPT_DIR}/config/sysmgr.conf"
...
_backup_restore() {
    ...
    # Kernel
    if [[ -f "$restore_source/99-sysmgr-hardening.conf" ]]; then
        run_cmd cp "$restore_source/99-sysmgr-hardening.conf" /etc/sysctl.d/
        run_cmd sysctl -p "$SYSCTL_CONF_FILE"
        ok "Настройки ядра восстановлены."
    fi
    
    # Sysmgr config
    if [[ -f "$restore_source/sysmgr.conf" ]]; then
        run_cmd cp "$restore_source/sysmgr.conf" "${SCRIPT_DIR}/config/"
        ok "Главный конфиг sysmgr.conf восстановлен."
    fi
...
_backup_cleanup() {
    ...
    backups_to_delete=$(ls -t "$BACKUP_DIR_SEC"/sysmgr-security-backup-*.tar.gz 2>/dev/null | tail -n +$((keep + 1)))
