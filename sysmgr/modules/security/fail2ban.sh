...
F2B_WHITELIST_FILE="/etc/sysmgr/fail2ban-whitelist.txt"
    ...
    if _f2b_is_service_active; then
        ...
        for jail in $jails_list; do
            ...
            case "$jail" in
                ...
                portscan-sysmgr) display_name="Порт-скан" ;;
                nginx-auth-sysmgr) display_name="Nginx Auth" ;;
                nginx-bots-sysmgr) display_name="Nginx Bots" ;;
                nginx-scanners-sysmgr) display_name="Nginx Scan" ;;
...
_f2b_whitelist_menu() {
    ...
    local global_file="/etc/sysmgr/global-whitelist.txt"
    while true; do
...
_f2b_extended_menu() {
    while true; do
        ...
        _f2b_is_jail_enabled "portscan-sysmgr" && portscan_status="(${C_GREEN}вкл${C_RESET})"
        
        local nginx_auth_status="(${C_RED}выкл${C_RESET})"
        _f2b_is_jail_enabled "nginx-auth-sysmgr" && nginx_auth_status="(${C_GREEN}вкл${C_RESET})"
        
        local nginx_bots_status="(${C_RED}выкл${C_RESET})"
        _f2b_is_jail_enabled "nginx-bots-sysmgr" && nginx_bots_status="(${C_GREEN}вкл${C_RESET})"

        local nginx_scanners_status="(${C_RED}выкл${C_RESET})"
        _f2b_is_jail_enabled "nginx-scanners-sysmgr" && nginx_scanners_status="(${C_GREEN}вкл${C_RESET})"
        ...
        case "$choice" in
            ...
            1) 
                ...
                _f2b_jail_submenu "portscan-sysmgr" "syslog" "$f" "any" "ufw[name=portscan,port=any,protocol=tcp]" "Защита от сканирования портов на основе логов UFW."
                ;;
            2) 
                local f="[Definition]\nfailregex = ^.*\\[error\\] \\d+#\\d+: \\*\\d+ user \"\\S+\":? (password mismatch|was not found in).*, client: <HOST>, server: \\S+, request: \"\\S+ \\S+ HTTP/\\d+\\.\\d+\", host: \"\\S+\"\nignoreregex ="
                _f2b_jail_submenu "nginx-auth-sysmgr" "nginx-error" "$f" "any" "ufw[name=nginx-auth,port=any,protocol=tcp]" "Защита от подбора паролей HTTP Basic Auth в Nginx."
                ;;
            3) 
                ...
                _f2b_jail_submenu "nginx-bots-sysmgr" "nginx-access" "$f" "any" "ufw[name=nginx-bots,port=any,protocol=tcp]" "Блокировка подозрительных ботов и парсеров в Nginx."
                ;;
            4) 
                ...
                _f2b_jail_submenu "nginx-scanners-sysmgr" "nginx-access" "$f" "any" "ufw[name=nginx-scanners,port=any,protocol=tcp]" "Защита от сканирования уязвимостей и админок (404/403 ошибки)."
                ;;
            c|C)
                local custom_name
...
EOF
            ...
            d|D)
                ...
                run_cmd sed -i "/^\[portscan-sysmgr\]/,/^\s*\[/ s/enabled\s*=\s*true/enabled = false/" /etc/fail2ban/jail.local 2>/dev/null
                run_cmd sed -i "/^\[nginx-auth-sysmgr\]/,/^\s*\[/ s/enabled\s*=\s*true/enabled = false/" /etc/fail2ban/jail.local 2>/dev/null
                run_cmd sed -i "/^\[nginx-bots-sysmgr\]/,/^\s*\[/ s/enabled\s*=\s*true/enabled = false/" /etc/fail2ban/jail.local 2>/dev/null
                run_cmd sed -i "/^\[nginx-scanners-sysmgr\]/,/^\s*\[/ s/enabled\s*=\s*true/enabled = false/" /etc/fail2ban/jail.local 2>/dev/null
                _f2b_ensure_jail_logs_exist
                run_cmd systemctl reload fail2ban 2>/dev/null
                ;;
            b|B) break ;;
...
_f2b_detect_nginx_log() {
    ...
    local extra_logs=()
    mapfile -t extra_logs < <(find /var/log /opt /home -maxdepth 4 -type f -name "*.log" \( -path "*nginx*" -o -path "*sysmgr*" -o -path "*remnawave*" -o -path "*proxy*" \) 2>/dev/null | head -25)
...
_f2b_reinstall() {
    ...
    local backup_dir="/etc/sysmgr/backups/fail2ban"
