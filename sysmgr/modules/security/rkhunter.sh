...
RKHUNTER_CRON_FILE="/etc/cron.weekly/sysmgr-rkhunter-scan"
...
#!/bin/bash
# Sysmgr Security Module: Weekly rkhunter scan
(
rkhunter --update --quiet
rkhunter --check --cronjob --report-warnings-only
) &> /var/log/sysmgr_rkhunter_last.log
CRON
            run_cmd chmod +x "$RKHUNTER_CRON_FILE"
            ok "Еженедельное сканирование включено."
            warn "Результаты будут сохраняться в /var/log/sysmgr_rkhunter_last.log"
        fi
    fi
}
