...
GEO_CONFIG_DIR="/etc/sysmgr/geoblock"
GEO_COUNTRIES_FILE="${GEO_CONFIG_DIR}/countries.txt"
GEO_IPSET_NAME="sysmgr_geoblock"
GEO_SERVICE_FILE="/etc/systemd/system/sysmgr-geoblock.service"
GEO_RESTORE_SCRIPT="/usr/local/bin/sysmgr-geoblock-restore.sh"
GLOBAL_WHITELIST_FILE="/etc/sysmgr/global-whitelist.txt"
...
_geo_get_cron_schedule() {
    local cron_file="/etc/cron.d/sysmgr-geoblock-update"
    if [[ -f "$cron_file" ]]; then
        local line
...
_geo_show_status() {
    ...
    if run_cmd systemctl is-enabled sysmgr-geoblock &>/dev/null 2>&1; then
        autostart="${C_GREEN}Включена${C_RESET}"
    fi

    local autoupdate="${C_GRAY}Выключено${C_RESET}"
    if [[ -f "/etc/cron.d/sysmgr-geoblock-update" ]]; then
        local sched
...
_geo_activate() {
    ...
    local temp_wl_ipset="sysmgr_geo_whitelist_temp"
        ...
        while [[ ${#pids[@]} -lt $max_parallel && $idx -lt $total_countries ]]; do
            ...
            (
                ...
                if [[ ! -s "$tmp_file" ]] || ! tr -d '\r' < "$tmp_file" | grep -q "^[0-9]"; then
                    curl -s -L -H "User-Agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36" --max-time 15 "${GH_PROXY:-https://gh-proxy.com/}https://raw.githubusercontent.com/ipverse/country-ip-blocks/master/country/${country,,}/ipv4-aggregated.txt" > "$tmp_file"
                fi
    ...
    if ! command -v global_whitelist_prepend_system_ips &>/dev/null; then
        if [[ -f "${SCRIPT_DIR:-/opt/sysmgr}/modules/security/whitelist_manager.sh" ]]; then
            source "${SCRIPT_DIR:-/opt/sysmgr}/modules/security/whitelist_manager.sh" && found_wl_manager=1
    ...
    # Swap whitelist
    if ! run_cmd ipset list sysmgr_geo_whitelist &>/dev/null; then
        run_cmd ipset create sysmgr_geo_whitelist hash:net hashsize 256 maxelem 1024 2>/dev/null || true
    fi
    run_cmd ipset swap sysmgr_geo_whitelist "$temp_wl_ipset"
    run_cmd ipset destroy "$temp_wl_ipset" 2>/dev/null || true
    ...
    run_cmd ipset save sysmgr_geo_whitelist | run_cmd tee "$GEO_CONFIG_DIR/sysmgr_geo_whitelist.ipset" > /dev/null
...
_geo_deactivate() {
    ...
    run_cmd ipset destroy sysmgr_geo_whitelist 2>/dev/null || true

    # Удаляем правило из before.rules
    _geo_remove_ufw_rule

    # Удаляем автозагрузку
    run_cmd systemctl disable sysmgr-geoblock 2>/dev/null || true
...
geo_block = """
# --- НАЧАЛО: Sysmgr Geo-Block ---
# Блокировка по странам (новые соединения, исключая loopback и белый список)
-A ufw-before-input ! -i lo -m set --match-set ${GEO_IPSET_NAME} src -m set ! --match-set sysmgr_geo_whitelist src -m conntrack --ctstate NEW -j DROP
# --- КОНЕЦ: Sysmgr Geo-Block ---
"""
...
with open('/etc/ufw/before.rules', 'r') as f:
    content = f.read()
content = re.sub(r'\n# --- НАЧАЛО: Sysmgr Geo-Block ---.*?# --- КОНЕЦ: Sysmgr Geo-Block ---\n', '', content, flags=re.DOTALL)
...
#!/bin/bash
# Sysmgr Geo-Block: Быстрое оффлайн-восстановление ipset после ребута
...
ipset destroy sysmgr_geo_whitelist 2>/dev/null || true

# 1. Проверяем локальный кэш
if [[ -f "${GEO_CONFIG_DIR}/${GEO_IPSET_NAME}.ipset" && -f "${GEO_CONFIG_DIR}/sysmgr_geo_whitelist.ipset" ]]; then
    echo "[i] Восстановление Geo-Block из локального кэша..."
    if ipset restore < "${GEO_CONFIG_DIR}/${GEO_IPSET_NAME}.ipset" && \
       ipset restore < "${GEO_CONFIG_DIR}/sysmgr_geo_whitelist.ipset"; then
...
ipset create sysmgr_geo_whitelist hash:net hashsize 256 maxelem 1024 2>/dev/null || true
...
while IFS= read -r country || [[ -n "$country" ]]; do
    ...
    if [[ -z "$ZONE_DATA" ]] || ! echo "$ZONE_DATA" | tr -d '\r' | grep -q "^[0-9]"; then
        ZONE_DATA=$(curl -s -L -H "User-Agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36" --max-time 15 "${GH_PROXY:-https://gh-proxy.com/}https://raw.githubusercontent.com/ipverse/country-ip-blocks/master/country/${country}/ipv4-aggregated.txt" 2>/dev/null)
    fi
    if [[ -z "$ZONE_DATA" ]] || ! echo "$ZONE_DATA" | tr -d '\r' | grep -q "^[0-9]"; then
...
if [[ -f "${GLOBAL_WHITELIST_FILE}" ]]; then
    ...
    grep -v '^\s*#' "${GLOBAL_WHITELIST_FILE}" | grep -v '^\s*$' | awk '{print $1}' | while read -r ip; do
        ip="${ip//[$'\r\n\t ']/}"
        if [[ -n "$ip" && "$ip" != *:* ]]; then
            echo "add sysmgr_geo_whitelist $ip -exist" >> "$TEMP_WL_RESTORE"
        fi
    done
...
[Unit]
Description=Sysmgr Geo-Block Restore
...
SERVICE
    ...
    run_cmd systemctl enable sysmgr-geoblock 2>/dev/null || true
...
_geo_test_ip() {
    ...
    if run_cmd ipset list sysmgr_geo_whitelist &>/dev/null; then
        is_active=1
        if ipset test sysmgr_geo_whitelist "$ip" &>/dev/null; then
...
_geo_write_updater_script() {
    local updater_script="/usr/local/bin/sysmgr-geoblock-update.sh"
    
    cat <<'UPDATE_SCRIPT' | run_cmd tee "$updater_script" > /dev/null
#!/bin/bash
# Sysmgr Geo-Block: Тихое фоновое автообновление правил
export SCRIPT_DIR="/opt/sysmgr"
source "${SCRIPT_DIR}/modules/core/common.sh" 2>/dev/null || true
source "${SCRIPT_DIR}/modules/security/geoblock.sh" 2>/dev/null || true

# Временный лог
exec 1>>/var/log/sysmgr-geoblock-update.log 2>&1

echo "[$(date '+%Y-%m-%d %H:%M:%S')] Запуск автообновления..."

if [[ ! -f "/etc/sysmgr/geoblock/countries.txt" ]] || [[ ! -s "/etc/sysmgr/geoblock/countries.txt" ]]; then
...
while IFS= read -r line || [[ -n "$line" ]]; do
    ...
    line="${line^^}"
    [[ -n "$line" ]] && countries+=("$line")
done < "/etc/sysmgr/geoblock/countries.txt"

total_countries=${#countries[@]}
temp_dir=$(mktemp -d)
temp_ipset="sysmgr_geoblock_temp"
...
if [[ "$subnets_total" -gt 0 && -f "$temp_dir/restore.txt" ]]; then
    if ipset restore < "$temp_dir/restore.txt"; then
        # Swap
        if ! run_cmd ipset list sysmgr_geoblock &>/dev/null; then
            ipset create sysmgr_geoblock hash:net hashsize 65536 maxelem 500000
        fi
        ipset swap sysmgr_geoblock "$temp_ipset"
        ipset destroy "$temp_ipset" 2>/dev/null || true
        
        # Сохраняем в кэш
        mkdir -p "/etc/sysmgr/geoblock"
        ipset save sysmgr_geoblock > "/etc/sysmgr/geoblock/sysmgr_geoblock.ipset"
