...
# --- Конфигурация ---
GLOBAL_WHITELIST_DIR="/etc/sysmgr"
...
# ══════════════════════════════════════════════════════════
# Глобальный Белый Список IP (Sysmgr Unified Whitelist)
# ══════════════════════════════════════════════════════════
    ...
    for ip_key in "${ordered_keys[@]}"; do
        ...
        if [[ "$first_entry" -eq 1 ]]; then
            if ! echo "$current_content" | grep -q "СИСТЕМНЫЕ IP — НЕ УДАЛЯТЬ"; then
                new_block+="# ─────────────────────────────────────────────────────────────────
# ⚠️  СИСТЕМНЫЕ IP — НЕ УДАЛЯТЬ! Добавлено автоматически Sysmgr.
...
_gwl_sync_fail2ban() {
    ...
    run_cmd cp -f "$GLOBAL_WHITELIST_FILE" "/etc/sysmgr/fail2ban-whitelist.txt" 2>/dev/null || true
...
_gwl_sync_shaper() {
    # Ищем конфиг шейпера
    local shaper_config_dir="/etc/sysmgr/traffic_limiter"
    local shaper_whitelist="${shaper_config_dir}/global-whitelist.txt"
    local ctrl_py="${SCRIPT_DIR}/modules/local/sysmgr_ctrl.py"
    local pin_dir="/sys/fs/bpf/sysmgr/maps"

    if [[ ! -f "$ctrl_py" ]]; then
        debug_log "GWL_SYNC: sysmgr_ctrl.py не найден, пропуск шейпера."
        return
    fi
...
_gwl_sync_geoblock() {
    ...
    for family in "inet" "inet6"; do
        local set_name="sysmgr_geo_whitelist"
        [[ "$family" == "inet6" ]] && set_name="sysmgr_geo_whitelist6"
    ...
    for ip in "${ips[@]}"; do
        if [[ "$ip" == *":"* ]]; then
            run_cmd ipset add sysmgr_geo_whitelist6 "$ip" 2>/dev/null || true
        else
            run_cmd ipset add sysmgr_geo_whitelist "$ip" 2>/dev/null || true
        fi
    done
...
# Добавляет whitelist IP в блок Sysmgr Anti-DDoS если он уже настроен
_gwl_sync_ufw() {
    local ips=("$@")
    local before_rules="/etc/ufw/before.rules"
    local antiddos_start="# --- НАЧАЛО: Sysmgr Anti-DDoS ---"
...
with open('/etc/ufw/before.rules', 'r') as f:
    content = f.read()
target = '# --- НАЧАЛО: Sysmgr Anti-DDoS ---'
...
show_global_whitelist_menu() {
    ...
    while true; do
        ...
        # Шейпер
        if [[ -d "/sys/fs/bpf/sysmgr/maps" ]]; then
        ...
        # Geo-block
        if command -v ipset &>/dev/null && ipset list sysmgr_geo_whitelist -terse &>/dev/null; then
