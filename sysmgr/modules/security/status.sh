...
show_full_security_status() {
    ...
    if command -v ipset &>/dev/null && run_cmd ipset list sysmgr_geoblock -terse &>/dev/null 2>&1; then
        local geo_count
        geo_count=$(run_cmd ipset list sysmgr_geoblock -terse 2>/dev/null | grep -Fi "Number of entries:" | awk '{print $4}' || echo "0")
        print_key_value "Статус" "${C_GREEN}Активен${C_RESET}" "$LABEL_WIDTH"
    ...
    if [[ -f "/etc/sysmgr/global-whitelist.txt" ]]; then
        local wl_count
        wl_count=$(grep -v '^\s*#' /etc/sysmgr/global-whitelist.txt 2>/dev/null | grep -vc '^\s*$' || echo "0")
    ...
    if [[ -f "/etc/sysctl.d/99-sysmgr-hardening.conf" ]]; then
        local syn_cookies
    ...
    if ! command -v rkhunter &> /dev/null; then
        ...
        if [[ -f "/etc/cron.weekly/sysmgr-rkhunter-scan" ]]; then
