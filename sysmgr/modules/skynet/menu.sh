...
_skynet_add_server_wizard() {
    ...
    case "$k_choice" in
        ...
        4)
            _import_ssh_key
            final_key=$(ls -t "${HOME}/.ssh/sysmgr_imported_"* 2>/dev/null | head -n1)
            if [[ -z "$final_key" ]]; then
...
_show_server_management_menu() {
    ...
    _sm_connect() {
        ...
        _skynet_push_context() {
            local gwl_file="/etc/sysmgr/global-whitelist.txt"
            local ext_file="/etc/sysmgr/ext_services/scripts.db"
            ...
            local tmp_ctx; tmp_ctx=$(mktemp /tmp/sysmgr_ctx_XXXXXX.sh)
            {
                echo '#!/bin/bash'
...
if [[ -n "$GWL_B64" ]]; then
    mkdir -p /etc/sysmgr
    gwl_file="/etc/sysmgr/global-whitelist.txt"
...
if [[ -n "$EXT_B64" ]]; then
    mkdir -p /etc/sysmgr/ext_services
    local_db="/etc/sysmgr/ext_services/scripts.db"
...
REMOTE_SCRIPT
            ...
            if scp -q -P "$s_port" -F /dev/null -o IdentitiesOnly=yes -i "$s_key" \
               ...
               "$tmp_ctx" "${s_user}@${s_ip}:/tmp/sysmgr_ctx.sh" 2>/dev/null; then
                
                # Используем run_remote, чтобы корректно обработать NOPASSWD sudo
                run_remote "bash /tmp/sysmgr_ctx.sh; rm -f /tmp/sysmgr_ctx.sh" >/dev/null 2>&1
            fi
            rm -f "$tmp_ctx"
        }
        ...
        if [[ -z "$remote_ver" ]] || _skynet_is_local_newer "$VERSION" "$remote_ver"; then
            ...
            local install_cmd="export SYSMGR_NO_AUTOSTART=1; wget -qO /tmp/i.sh ${INSTALLER_URL_RAW} && bash /tmp/i.sh"
        ...
        local remote_exec_command="bash -l -c 'SKYNET_MODE=1 /opt/sysmgr/sysmgr.sh'"
