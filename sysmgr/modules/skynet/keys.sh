...
_select_existing_ssh_key() {
    ...
    for k in "${HOME}/.ssh/${SKYNET_UNIQUE_KEY_PREFIX}"* "${HOME}/.ssh/sysmgr_imported_"*; do
        if [[ -f "$k" ]] && [[ "$k" != *.pub ]]; then
            ...
            elif [[ "$k_name_full" == "sysmgr_imported_"* ]]; then
                display_label="Импортированный ключ (${k_name_full#sysmgr_imported_})"
            fi
...
# Imports an existing SSH key pair into Sysmgr's management using nano
_import_ssh_key() {
    clear
    ...
    local target_private_path="${HOME}/.ssh/sysmgr_imported_${key_name}"
    ...
    if ! ssh-keygen -y -f "$tmp_file" >/dev/null 2>&1; then
        ...
        printf_description "Sysmgr поддерживает форматы OpenSSH, RSA, ED25519."
        rm -f "$tmp_file"
        sleep 3
        return
    fi
    ...
    printf_info "Имя в системе: sysmgr_imported_${key_name}"
    ...
    while true; do
        ...
        for k in "${HOME}/.ssh/${SKYNET_UNIQUE_KEY_PREFIX}"* "${HOME}/.ssh/sysmgr_imported_"*; do
            if [[ -f "$k" ]] && [[ "$k" != *.pub ]]; then
                ...
                if [[ "$k_name_full" == "sysmgr_imported_"* ]]; then
                    key_type_color="${C_CYAN}"
                    key_type_label="IMPORTED KEY"
                    display_label="(${k_name_full#sysmgr_imported_})"
                fi
        ...
        if [ ${#formatted_key_lines[@]} -gt 0 ]; then
            ...
            printf_info "Не найдено ключей, управляемых Sysmgr."
        ...
        printf_description "     - Добавляет ваш существующий ключ в управление Sysmgr."
