...
_skynet_run_plugin_on_server() {
    ...
    if ! scp -q "${ssh_opts[@]}" "$plugin" "${user}@${ip}:/tmp/sysmgr_plugin.sh"; then
        err "Не удалось скопировать плагин на $name (timeout/access)."
        return 1
    fi

    local run_cmd="bash /tmp/sysmgr_plugin.sh; rm -f /tmp/sysmgr_plugin.sh"
...
_skynet_run_plugin_on_server_with_env() {
    ...
    local run_cmd="${env_vars} bash /tmp/sysmgr_plugin.sh; rm -f /tmp/sysmgr_plugin.sh"
...
_skynet_run_plugin_for_capture() {
    ...
    local port="$6"
    local key_path="$7"
    local temp_plugin_path="/tmp/sysmgr_plugin_$$_${RANDOM}"
