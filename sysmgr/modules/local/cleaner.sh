...
# --- Конфигурация ---
CLEANER_CONF="/etc/sysmgr/cleaner"
...
_inspect_directory() {
    ...
    while true; do
        ...
        echo ""
        du -sh "$dir"/* 2>/dev/null | sort -hr | head -10 > /tmp/sysmgr_du.txt
        if [ ! -s /tmp/sysmgr_du.txt ]; then printf_warning "Пусто или нет доступа."; declare -a paths=(); else
            ...
            while read -r line; do
                ...
                paths[$i]="$fpath"; ((i++))
            done < /tmp/sysmgr_du.txt; rm -f /tmp/sysmgr_du.txt
        fi
        echo ""
...
_lr_create_rule() {
    ...
    local file="/etc/logrotate.d/sysmgr_${name}"
...
_manage_logrotate() {
    while true; do
        ...
        case $choice in
            ...
            3) clear; menu_header "Активные правила"; ls /etc/logrotate.d/sysmgr_* 2>/dev/null; wait_for_enter ;;
            4) 
