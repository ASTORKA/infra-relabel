...
readonly TL_CONFIG_DIR="/etc/sysmgr/traffic_limiter"
readonly TL_BPF_SRC_PATH="${SCRIPT_DIR}/modules/local/shaper.bpf.c"
readonly TL_BPF_OBJ_PATH="${TL_CONFIG_DIR}/shaper.bpf.o"
readonly TL_CTRL_PY_PATH="${SCRIPT_DIR}/modules/local/sysmgr_ctrl.py"
readonly TL_SERVICE_NAME="sysmgr-traffic-limiter.service"
readonly TL_SERVICE_PATH="/etc/systemd/system/${TL_SERVICE_NAME}"
readonly TL_OLD_APPLY_SCRIPT="/usr/local/bin/sysmgr-traffic-limiter-apply.sh"
readonly TL_BPF_PIN_DIR="/sys/fs/bpf/sysmgr"
...
_tl_show_shaper_intro() {
    echo -e "  ${C_CYAN}╔══════════════════════════════════════════════════════════╗${C_RESET}"
    echo -e "  ${C_CYAN}║${C_RESET}  ${C_YELLOW}⚡ Sysmgr eBPF Traffic Shaper v3.2 (HighLoad)${C_RESET}"
    echo -e "  ${C_CYAN}╠══════════════════════════════════════════════════════════╣${C_RESET}"
...
_tl_generate_ebpf_service_file() {
    ...
    cat <<EOF
[Unit]
Description=Sysmgr eBPF Traffic Limiter (Multi-Rule)
...
ExecStartPost=-${python_path} ${TL_CTRL_PY_PATH} --pin-dir ${PIN_MAPS} whitelist-sync --file ${TL_CONFIG_DIR}/whitelist.txt /etc/sysmgr/global-whitelist.txt
