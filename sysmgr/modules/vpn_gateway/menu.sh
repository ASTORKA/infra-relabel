#!/bin/bash
# ============================================================ #
# ==        VPN GATEWAY MODULE: ДЛЯ SYSMGR-ECOSYSTEM        == #
# ============================================================ #
#
...
# ── Сертификаты ───────────────────────────────────────────────────
# Сохраняет сертификаты в /etc/sysmgr-bedolaga/certs/<domain>/ после выпуска/продления
...
_vgw_detect_cert_source() {
    ...
    local domain_backup=""
    if [[ -n "$domain" ]]; then
        domain_backup="/etc/sysmgr-bedolaga/certs/${domain}/fullchain.pem"
    fi
    if [[ -f "${live_certs_dir}/fullchain.pem" || ( -n "$domain_backup" && -f "$domain_backup" ) || -f /etc/sysmgr-bedolaga/certs/fullchain.pem ]]; then
        echo "sysmgr:${live_certs_dir}:/etc/nginx/certs"
        return 0
    fi
    echo "none"
}
...
# Сохраняет данные об инжектированном nginx-конфиге в /etc/sysmgr-bedolaga/
...
_vgw_detect_show_plan() {
    ...
    if [[ "$csrc" == "certwarden"* && "$CSRC_EFFECTIVE" == "sysmgr" ]]; then
        echo -e "  ${C}║${E}  SSL:        ${G}certwarden -> Let's Encrypt (sysmgr)${E} ${W}(не совпадает домен!)${E}"
    else
...
# с учётом csrc (certwarden / letsencrypt / sysmgr / none)
...
_vgw_resolve_ssl_paths() {
    ...
    case "$csrc_type" in
        certwarden)
            ...
            #   /etc/nginx/ssl/example.com   → домен: example.com
            #   /etc/nginx/ssl/lendinghello.mooo.com → домен: lendinghello.mooo.com
            #
            # Также поддерживаем Wildcard-сертификаты (например, если certwarden для домена example.com,
            # а public_domain это lendinghello.example.com — он подходит!).
...
_vgw_check_our_certs_exist() {
    ...
    if [[ ! -f "$cert_file" || ! -s "$cert_file" || ! -f "$key_file" || ! -s "$key_file" ]]; then
        ...
        local found_valid="0"
        for p_dir in "/etc/letsencrypt/live/${domain}" "/etc/sysmgr-bedolaga/certs/${domain}" "${certs_dir}/../letsencrypt/live/${domain}"; do
...
EOF
        ...
        if [[ -n "$cname" ]]; then
            ...
            echo -e "  Чтобы применить автоматические сертификаты Bedolaga / Sysmgr:"
            echo ""
    ...
    if [[ -n "$cname" ]]; then
        ...
        if [[ "$docker_mount_notice" == "1" ]]; then
            echo -e "  ${G}${B}👉 ИСПОЛЬЗОВАНИЕ АВТО-СЕРТИФИКАТОВ BEDOLAGA / SYSMGR${E}"
            ...
            echo ""
            echo -e "  ${W}${B}👉 ЕСЛИ ВЫ ХОТИТЕ ПЕРЕЙТИ НА АВТО-СЕРТИФИКАТЫ BEDOLAGA / SYSMGR${E}"
        ...
        echo "  ────────────────────────────────────────────────────"
        echo ""
    elif [[ "$CSRC_EFFECTIVE" == "sysmgr" && -z "$cname" ]]; then
...
print(c.get('quick_setup',{}).get('origin_domain','cabinet.example.com'))" 2>/dev/null || echo "cabinet.example.com")
    ...
    elif [[ -f "/etc/sysmgr-bedolaga/certs/${public_domain}/fullchain.pem" ]]; then
        resolved_cert_path="/etc/sysmgr-bedolaga/certs/${public_domain}"
    elif [[ -f "/etc/sysmgr-bedolaga/certs/fullchain.pem" ]]; then
        resolved_cert_path="/etc/sysmgr-bedolaga/certs"
...
vgw_manage_landing_pages() {
    ...
    while true; do
        ...
        echo -e "  • У вас есть кабинет (например: ${G}https://webcabinet.example.com/buy/wl-lte${E})."
