...
# @menu.manifest
#
# @item( main | d | 🗑️  Снести Решалу | _sysmgr_uninstall_wrapper | 90 | 90 | Удаляет все файлы и конфигурацию Решалы. )
#
...
if [ -f "${SCRIPT_DIR}/config/sysmgr.conf" ]; then
    source "${SCRIPT_DIR}/config/sysmgr.conf"
else
    echo "[FATAL ERROR] Configuration file config/sysmgr.conf not found."
    exit 1
fi
...
show_support_page() {
    ...
    local DON_PLATEGA_URL=""
    local DON_TRIBUT_URL=""
    
    local LINK_GROUP_URL="https://t.me/sysmgr_prod"
    local LINK_SITE_URL=""
    # ========================================================
    ...
    echo -e "  ${C_RED}  ╚═╝  ╚═╝╚══════╝╚══════╝╚═╝  ╚═╝╚═╝  ╚═╝╚══════╝╚═╝  ╚═╝${C_RESET}"
    echo
    echo -e "           ${C_BOLD}${C_WHITE}от ${C_YELLOW}admin${C_WHITE} с любовью блять!${C_RESET}"
    echo
