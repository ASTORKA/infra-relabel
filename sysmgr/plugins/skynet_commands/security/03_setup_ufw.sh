...
if [[ -f "/etc/sysmgr/global-whitelist.txt" ]]; then
    cat "/etc/sysmgr/global-whitelist.txt" >> "$temp_gwl"
fi
...
# Основной порт для VPN (стандарт Sysmgr)
...
if command -v docker &>/dev/null; then
    ...
    marker_start="# --- НАЧАЛО: Sysmgr Docker UFW Fix ---"
    marker_end="# --- КОНЕЦ: Sysmgr Docker UFW Fix ---"
